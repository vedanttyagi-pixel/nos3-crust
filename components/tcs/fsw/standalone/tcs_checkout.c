    ...
    switch (cc)
    {
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                ...
                if (status == OS_SUCCESS)
                {
                    OS_printf("TCS_RequestHK command success\n");
                }
                else
                {
                    OS_printf("TCS_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_TCS:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = TCS_RequestData(&TcsUart, &TcsData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("TCS_RequestData command success\n");
                }
                else
                {
                    OS_printf("TCS_RequestData command failed!\n");
                }
            }
            break;
...
/* Initialize HWLIB */
    ...
    // Close the device
    uart_close_port(&TcsUart);

#ifdef _NOS_ENGINE_LINK_
    nos_destroy_link();
#endif

    OS_printf("Cleanly exiting tcs application...\n\n");
    return 1;
}
